...
df$y <- round(df$y)
formula_nl <- bf(
  y | trials(n) ~ B + (A * (x - 17)^2 - B) * exp(-C * (x - 17)),
  A + B + C ~ 1,
  nl = TRUE
)
priors <- c(
  prior(normal(0.002, 0.05), nlpar = "A"),
  prior(normal(0.1, 0.05), nlpar = "B"),
  prior(normal(0.034, 0.05), nlpar = "C")
)
...
fit_brm <- brm(
  ...
  warmup = 1000,
  init = function() list(A = 0.002, B = 0.1, C = 0.03)
)
...
lines(age_seq, f_mean, col = "blue", lwd = 2)

points(df$x, df$y / df$n, col = "black", pch = 19)
