...
df$y <- round(df$y)
formula_nl <- bf(
  y | trials(n) ~ B + (A * (x - 16)^2 - B) * exp(-C * (x - 16)),
  A + B + C ~ 1,
  nl = TRUE
)
priors <- c(
  prior(normal(0.006, 0.003), nlpar = "A", lb = 0),  
  prior(normal(0.018, 0.007), nlpar = "B", lb = 0),  
  prior(normal(0.10,  0.05),  nlpar = "C", lb = 0)
)
fit_brm <- brm(
  formula = formula_nl,
  data = df,
  ...
  init = function() list(A = 0.17, B = 0.005, C = 0.93)
)
