...
df <- data.frame(
  ...
  y = c(3, 20, 30, 19, 8, 5),
  n = c(52, 113, 176, 171, 148, 128)
)
df$y <- round(df$y)
formula_nl <- bf(
  y | trials(n) ~ B + (A * (x - 20)^2 - B) * exp(-C * (x - 20)),
  A + B + C ~ 1,
  nl = TRUE
)
priors <- c(
  prior(normal(0.0145, 0.05), nlpar = "A"),
  prior(normal(-0.0032, 0.01), nlpar = "B"),
  prior(normal(0.2106, 0.01), nlpar = "C")
)
